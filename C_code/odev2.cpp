...
#include<locale.h>
	...
	int main(){
		...
		for(int i=0;i<5;i++){
			printf("%d.öðrencinin adý: ",i+1);scanf("%s",ogr[i].ad);
			printf("%d.öðrencinin soyadý: ",i+1);scanf("%s",ogr[i].soyad);
			printf("öðrencinin numarasý     : ");scanf("%s",&ogr[i].numara);
			for(int j=0;j<5;j++){
		...
		for(int i=0;i<5;i++){
			...
			printf("\n\t%d.dersin geçme notu:\t%2.2f",j+1,ogr[i].dersler[j].gecme_n=(float)(ogr[i].dersler[j].vize*0.4)+(ogr[i].dersler[j].final*0.6));
	}
			printf("\n---------------------------------------------------------------------------------------\n");
		}
	}
